...
function getInput() {
    ...
    let tempString = "{\"fname\": \"" + fname + "\", \"lname\": \"" + lname + "\", \"gender\": \"" + gender + "\", " + "\"email\": \"" + email + "\", \"dateNow\": \"" + dateNow + "\"}";
    debugger;
    let tempJson = new Array();
    if (fs.existsSync("records.json")) {
        tempJson = JSON.parse(fs.readFileSync("records.json").toString());
    }
    debugger;
    tempJson.push(JSON.parse(tempString));
    fs.writeFileSync("records.json",JSON.stringify(tempJson),);
    debugger;
