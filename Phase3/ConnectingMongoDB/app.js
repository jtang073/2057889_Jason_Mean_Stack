...
app.get('/addCourse', (res,req) => {
    ...
    let db = mongoose.connection;
    db.once("open",()=> {
        let tempJson = JSON.parse("{\"_id\":" + cid + ", \"cid\":" + cid + ", \"cname\": \"" + cname + "\", \"desc\":\"" + desc + "\", \"amount\":" + amount + "}");
        ...
        courseModel.insertMany([tempJson],(err,result)=> {
            if(!err){
                ...
                console.log("Failed to add course! Please use a UNIQUE course ID.");
            }
            mongoose.disconnect();
        });
    });
...
app.get('/updateCourse', (res,req) => {
    ...
    db.once("open",()=> {
        ...
        //console.log(tempJson)
        courseModel.updateOne(tempJson, {$set:{amount:newAmount}}, (err,result)=> {
            if(!err){
                console.log("Successfully updated course ID: " + cid + " to new amount:" + newAmount);
            }
            else {
                console.log(err);
            }
...
app.get('/deleteCourse', (res,req) => {
    ...
    db.once("open",()=> {
        ...
        courseModel.deleteMany(tempJson,(err,result)=> {
            if(!err){
                console.log("Successfully deleted course " + dcid + " from the database. (Did not do any error checking)");
            }
            else {
                console.log(err);
            }
